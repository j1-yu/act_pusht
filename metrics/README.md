...
| `raw/eval_C_nas20.json` | C 组：不开时间集成，`n_action_steps=20` | 同上 |
...
| `eval_results.json` | 上述 7 次的汇总：成功率、Wilson 95% CI、`avg_max_reward`、**逐 episode 原始分数**、Fisher 精确检验 | 由 `build_eval_results.py` 生成 |
...
脚本会重新读取 `raw/` 下的 7 个原始文件，重算成功率、Wilson 置信区间与 Fisher 精确检验，并覆盖写出 `eval_results.json`。输出示例：

```
  A_nas1           nas=1   te=None   1/50 =   2.0%  95%CI [0.3, 10.5]  avg_max_reward=0.1999
  B_te0.01_nas1    nas=1   te=0.01   0/50 =   0.0%  95%CI [0.0, 7.1]   avg_max_reward=0.3911
  C_nas20          nas=20  te=None  26/50 =  52.0%  95%CI [38.5, 65.2] avg_max_reward=0.9218
  ...
```
...
| 项 | 值 |
|---|---|
| 权重 | `outputs/train/act_pusht_std/checkpoints/last/pretrained_model`（= step 200000） |
...
**七次评估为什么可比**：同一份权重、同一 seed、同样 50 个 episode。LeRobot 由 `start_seed` 推出逐 episode 初始局面（`start_seed + i`），seed 固定即意味着**每次都面对完全相同的 50 个初始局面**，所以差异只来自推理设置。
