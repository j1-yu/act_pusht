...
可观察到的两点：loss 在后 3 万步基本走平（0.0435 → 0.0423），grad norm 持续单调下降 —— **但 loss 走平不等于成功率到顶**，因为中间 checkpoint 并没有评估（见下节）。
...
| 文件 | 内容 | 大小 |
|---|---|---|
| `metrics/raw/eval_*.json` | **7 次评估的原始产物**，含全部 50 个 episode 的逐条分数与成功标记 | 6.6 ~ 7.1 KB × 7 |
...
- 用 `lerobot-eval` 完成 **7 次评估**（3 组主对照 + 4 档系数扫描），全部 50 episodes / seed 1000 / 同一权重
- 保存 7 次评估的**逐 episode 原始数据**并提交入库
...
- **150K checkpoint 未评估**：多训的 5 万步究竟有没有换来成功率，目前无从判断
